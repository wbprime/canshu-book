 ...
 * @since 1.0.0
 */
public class Typesafes_SimplePath_FromJson_Spec extends Specification {
    @Shared
    String key1 = "k123"
    @Shared
    String key2 = "y234"
    ...
    def "test absent value"() {
        ...
        StringBuilder sb = new StringBuilder("{\"").append(key1).append("\":{");
    ...
    def "test null value"() {
        ...
        StringBuilder sb = new StringBuilder("{\"").append(key1).append("\":{");
        sb.append(String.format("\"%s\":null", key2))
        def config = ConfigFactory.parseReader(new StringReader(sb.append("}}").toString()))
    ...
    def "test boolean value"() {
        ...
        StringBuilder sb = new StringBuilder("{\"").append(key1).append("\":{");
        sb.append(String.format("\"%s\":%s", key2, val))
        def config = ConfigFactory.parseReader(new StringReader(sb.append("}}").toString()))
    ...
    def "test integer value"() {
        ...
        StringBuilder sb = new StringBuilder("{\"").append(key1).append("\":{");
        sb.append(String.format("\"%s\":%d", key2, val))
        def config = ConfigFactory.parseReader(new StringReader(sb.append("}}").toString()))
    ...
    def "test float value"() {
        ...
        StringBuilder sb = new StringBuilder("{\"").append(key1).append("\":{");
        sb.append(String.format("\"%s\":%.8f", key2, val))
        def config = ConfigFactory.parseReader(new StringReader(sb.append("}}").toString()))
